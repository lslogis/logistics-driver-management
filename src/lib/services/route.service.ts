...
export class RouteService {
  ...
  async deleteRoute(id: string): Promise<void> {
    ...
    if (existingRoute.trips.length > 0) {
      throw new Error('운행 기록이 있는 노선은 삭제할 수 없습니다. 비활성화 처리해주세요.')
    }
   ...
   * 노선 검색 (간단한 자동완성용)
   */
  async searchRoutes(query: string, limit = 10): Promise<Pick<RouteResponse, 'id' | 'name' | 'loadingPoint' | 'unloadingPoint'>[]> {
    const routes = await this.prisma.routeTemplate.findMany({
      where: {
        isActive: true,
        OR: [
          ...
          { unloadingPoint: { contains: query, mode: 'insensitive' } }
        ]
      },
      select: {
        id: true,
