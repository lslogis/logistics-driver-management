...
export class SettlementApiService {
  ...
  private async getSettlementItems(driverId: string, yearMonth: string): Promise<SettlementItem[]> {
    ...
    return trips.map(trip => ({
      ...
      status: trip.status,
      routeName: trip.routeTemplate?.name,
      loadingPoint: trip.routeTemplate?.loadingPoint || (trip.customRoute as any)?.loadingPoint,
      unloadingPoint: trip.routeTemplate?.unloadingPoint || (trip.customRoute as any)?.unloadingPoint,
      driverFare: trip.driverFare.toString(),
      billingFare: trip.billingFare.toString(),
      deductionAmount: trip.deductionAmount?.toString(),
      isSubstitute: trip.status === 'SUBSTITUTE',
      substituteDriverId: trip.substituteDriverId,
      substituteDriverName: trip.substituteDriver?.name,
