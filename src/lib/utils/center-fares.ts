...
import { VEHICLE_TYPE_OPTIONS, normalizeVehicleTypeName } from './vehicle-types'
    ...
    reader.onload = (e) => {
      try {
        ...
        for (let index = 0; index < rows.length; index++) {
          ...
          const normalizedVehicleTypeId = normalizeVehicleTypeName(vehicleTypeName)
          ...
          const vehicle = VEHICLE_TYPE_OPTIONS.find(v => v.id === normalizedVehicleTypeId)
          if (!vehicle) {
            errors.push(`${rowNumber}행: 유효하지 않은 차량톤수입니다 (입력값: ${vehicleTypeName}, 사용 가능한 값: ${VEHICLE_TYPE_OPTIONS.map(v => v.name).join(', ')})`)
            continue
          }
