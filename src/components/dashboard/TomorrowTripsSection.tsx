...
export function TomorrowTripsSection() {
  const [trips, setTrips] = useState<TomorrowTrip[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    async function fetchTomorrowTrips() {
      try {
        setLoading(true)
        setError(null)
        ...
        const response = await fetch(
          `/api/trips?dateFrom=${dateFrom}&dateTo=${dateTo}&page=1&limit=100`
        )
        ...
        console.error('Failed to fetch tomorrow trips:', err)
        ...
        {trips.length > 0 && (
          <div className="mt-4 pt-4 border-t border-slate-200">
            <div className="flex justify-between items-center text-sm text-slate-600">
              <span>
                예정 {trips.filter(t => t.status === 'SCHEDULED').length}건, 
                완료 {trips.filter(t => t.status === 'COMPLETED').length}건, 
                결행 {trips.filter(t => t.status === 'CANCELLED').length}건
              </span>
              <Link 
                href="/trips" 
