...
// 임포트 가능한 데이터 타입
export type ImportType = 'drivers' | 'loading-points' | 'fixed-routes' | 'vehicles' | 'routes' | 'trips'
...
export const IMPORT_TYPE_CONFIGS: Record<ImportType, ImportTypeConfig> = {
  ...
  'fixed-routes': {
    title: '고정노선 가져오기',
    description: 'CSV 또는 Excel 파일로 고정노선 정보를 일괄 등록할 수 있습니다.',
    templateFileName: '고정노선등록템플릿.csv',
    acceptedFileTypes: ['.csv', '.xlsx', '.xls'],
    maxFileSize: 10,
    sampleFields: ['노선명', '출발지', '도착지', '거리', '기본요금', '요일패턴'],
    icon: 'route'
  },
  vehicles: {
    title: '차량 가져오기',
