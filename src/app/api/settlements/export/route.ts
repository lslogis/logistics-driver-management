    ...
    try {
      ...
      settlements.forEach(settlement => {
        settlement.items.forEach(item => {
          const routeName = item.trip?.routeTemplate?.name || 
            (item.trip?.customRoute as any)?.loadingPoint && 
            (item.trip?.customRoute as any)?.unloadingPoint ? 
            `${(item.trip!.customRoute as any).loadingPoint}-${(item.trip!.customRoute as any).unloadingPoint}` : 
            '커스텀'
          ...
          detailData.push({
            ...
            '차량번호': item.trip?.vehicle.plateNumber || '',
            '노선': routeName,
