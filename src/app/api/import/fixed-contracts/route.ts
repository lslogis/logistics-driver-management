...
function parseOperatingDays(dayString: string): number[] {
  ...
  for (const day of cleanString.split(',')) {
    ...
    console.log('🔍 [parseOperatingDays] Checking day:', trimmedDay, 'Found in map:', WEEKDAY_MAP.hasOwnProperty(trimmedDay))
...
function csvRowToFixedContract(row: any) {
  console.log('🔍 [csvRowToFixedContract] Raw row data:', JSON.stringify(row, null, 2))
  ...
  // 계약형태 변환
  const centerContractType = CONTRACT_TYPE_MAP[rawData.centerContractType] || rawData.centerContractType
  const driverContractType = rawData.driverContractType ? (CONTRACT_TYPE_MAP[rawData.driverContractType] || rawData.driverContractType) : centerContractType
  ...
  async (req: NextRequest) => {
    ...
    try {
      ...
      if (parseResult.errors.length > 0) {
        console.log('❌ [Fixed Contracts Import] File parsing failed:', parseResult.errors)
        return NextResponse.json({
          ok: false,
          error: {
      ...
      // 헤더 검증
      console.log('📋 [Fixed Contracts Import] CSV headers found:', Object.keys(csvData[0]))
      console.log('📋 [Fixed Contracts Import] Required headers:', REQUIRED_HEADERS)
      
      const headerErrors = validateHeaders(csvData, REQUIRED_HEADERS)
      if (headerErrors.length > 0) {
        console.log('❌ [Fixed Contracts Import] Header validation failed:', headerErrors)
      ...
      for (let i = 0; i < csvData.length; i++) {
        ...
        try {
          ...
          if (!contractData.centerName) {
            errors.push({ row: rowIndex, error: '센터명은 필수입니다', data: row })
            continue
          }
          
          if (!contractData.routeName) {
            errors.push({ row: rowIndex, error: '노선명은 필수입니다', data: row })
            continue
          }
          
          if (!contractData.operatingDays || contractData.operatingDays.length === 0) {
            errors.push({ row: rowIndex, error: '운행요일은 필수입니다', data: row })
            continue
          }
          
          if (!contractData.centerContractType) {
            errors.push({ row: rowIndex, error: '센터계약은 필수입니다', data: row })
            continue
          }
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            results: {
              ...
              imported: 0,
              errors,
              preview: validContracts.slice(0, 5)
            }
          }
        })
      }
          ...
          try {
            ...
            if (contractData.driverName) {
              ...
              if (contractData.vehicleNumber) {
                searchConditions.push({ vehicleNumber: contractData.vehicleNumber })
              }
            ...
            await prisma.fixedContract.create({
              data: {
                ...
                driverContractType: contractData.driverContractType || contractData.centerContractType,
