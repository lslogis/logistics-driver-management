...
export function useValidateTripsCSV() {
  return useMutation<ImportResponse, Error, File>({
    mutationFn: (file: File) => importsAPI.validateTripsCSV(file),
    onSuccess: (data) => {
      const results = data.data.results
      toast.success(`검증 완료: ${results.valid}개 유효, ${results.invalid}개 오류`)
    },
...
export function useImportTripsCSV() {
  ...
  return useMutation<ImportResponse, Error, File>({
    mutationFn: (file: File) => importsAPI.importTripsCSV(file),
    onSuccess: (data) => {
      const results = data.data.results
      toast.success(`가져오기 완료: ${results.imported}개 운행이 등록되었습니다`)
...
export function useTripsImportWorkflow() {
