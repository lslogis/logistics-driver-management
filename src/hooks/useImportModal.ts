...
import { 
  ...
  useFixedRoutesImportWorkflow,
  useVehiclesImportWorkflow,
  useRoutesImportWorkflow,
  useTripsImportWorkflow
...
export function useImportModal(type: ImportType) {
  ...
  const fixedRoutesWorkflow = useFixedRoutesImportWorkflow()
  const vehiclesWorkflow = useVehiclesImportWorkflow()
  const routesWorkflow = useRoutesImportWorkflow()
  const tripsWorkflow = useTripsImportWorkflow()
  ...
  const getWorkflow = useCallback(() => {
    switch (type) {
      ...
      case 'fixed-routes':
        return fixedRoutesWorkflow
      case 'vehicles':
        return vehiclesWorkflow
      case 'routes':
        return routesWorkflow
      case 'trips':
        return tripsWorkflow
      default:
        throw new Error(`Unsupported import type: ${type}`)
    }
  }, [type, driversWorkflow, loadingPointsWorkflow, fixedRoutesWorkflow, vehiclesWorkflow, routesWorkflow, tripsWorkflow])
